...
var Apple=function(){}
Apple.prototype={position:8,respawn:function(e,t,i){function s(e,t,i){return t>i?e>=i&&e<=t:e<=t||e>=i}for(var a=e%i,o=(e-t)%i+1,n=util.randomInt(20);s(n,a,o);)n=util.randomInt(20),console.log(n)
...
var l=this.tileSize/16,i=(snake.position-snake.length+1)%this.worldLength,t=this.snakeBits.create(i*this.tileSize+this.xOffset,this.yPosition,"snake")
t.scale.set(l,l),this.blopp.play(),snake.length<this.worldLength?(console.log("heeej"),apple.respawn(snake.position,snake.length,this.worldLength),this.appleBit.position.x=apple.position*this.tileSize+this.xOffset):(console.log("bla"),this.appleBit.destroy())}}else this.gameOver},timeToTick:function(){var e=!1
